...
from osocrNG.ocr_modules_NG.sampler_NG.temporal_att_NG_mk1 import neko_vmsk
...
class temporal_fe_fpn_supres(temporal_fe_fpn):
    PARAM_extra_deconv="extra_decov";
    def __init__(this, params):
        super(temporal_fe_fpn_supres, this).__init__(params);
        num_channels=params[this.PARAM_num_channels];
        num_extra_deconvs=params[this.PARAM_extra_deconv];
        deconvs=[nn.BatchNorm2d(num_channels),nn.ReLU()]
        for i in range(1, num_extra_deconvs):
            blk=[nn.ConvTranspose2d(num_channels, num_channels,
                                                           3,
                                                           2,
                                                           1),
                                         nn.BatchNorm2d(num_channels),
                                         nn.ReLU(True)];
            deconvs+=blk;
        deconvs.append(nn.ConvTranspose2d(num_channels, num_channels,
                                                           3,
                                                           2,
                                                           1));
        this.xdeconvs = nn.Sequential(*deconvs);
    def forward(this,input):
        x,stoppers = super().forward(input);
        return this.deconvs(x),stoppers;
        for i in range(0, len(this.fpn)):
            x = this.fpn[i](x) + input[i + 1]
        conv_feats = []
        stoppers = []
