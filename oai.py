...
def ocr(pilimg: Image.Image, sk: str) -> str:
    """Sends a single PIL image to OpenAI for text extraction."""
    client = OpenAI(api_key=sk)

    # Convert PIL to Base64
    buffer = io.BytesIO()
    pilimg.save(buffer, format="JPEG", quality=85)
    base64_image = base64.b64encode(buffer.getvalue()).decode('utf-8')

    response = client.chat.completions.create(
        model="gpt-5.2-2025-12-11",  # Or "gpt-4o-mini" for 10x lower cost
        seed=9,
        temperature=0,
        messages=[
            {
                ...
                "content": [
                    {"type": "text", "text": """
                    Do OCR and return only transcription (modern Yi). 
...
def run_sequential_ocr(target_folder, api_key):
    # 1. Find all images
    files = sorted(glob.glob(os.path.join(target_folder, "*.[jp][pn]g")))  # Matches .jpg, .jpeg, .png

    print(f"--- Starting Sequential Job: {len(files)} files found ---")
    ...
    for i, file_path in tqdm.tqdm(enumerate(files, 1)):
        ...
        try:
            ...
            with Image.open(file_path) as img:
                # Ensure image is in RGB (OpenAI doesn't like some PNG formats)
                text = ocr(img.convert("RGB"), api_key)

                # 3. Save the result to a .txt file immediately
...
MY_SK = <your sk here>
run_sequential_ocr("sampled_images", MY_SK)
