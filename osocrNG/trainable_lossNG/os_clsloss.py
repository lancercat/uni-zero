...
class abstract_cent(nn.Module):
    ...
    DFT_reduction = "mean";
    DFT_ignore_index = -1;
    PARAM_label_smooting="label_smooting";
    def __init__(this, param):
        super(abstract_cent, this).__init__();
        this.setuploss(param);

    def setuploss(this, param):
        this.smooth=neko_get_arg(this.PARAM_label_smooting,param,0.0);
        this.criterion_CE = nn.CrossEntropyLoss(label_smoothing=this.smooth);
        # this.aceloss=
...
class oslenlossNG(abstract_cent):
    def forward(this, lencls, gtlen):
        ...
        return lenloss;
class osclsNG_perinstance_d(abstract_cent):
    DFT_reduction = "none";
    def setuploss(this, param):
        super().setuploss(param);
        this.fakelogit=nn.Parameter(torch.zeros([3]));
    def forward(this, outcls, label_flatten,instmap):
        w = torch.ones(outcls.shape[-1]).to(outcls.device,dtype=outcls.dtype);
        # w[-1] = 0;
        this.fakelogit=nn.Parameter(outcls.detach()+0);
        clsloss_f = trnf.cross_entropy(this.fakelogit, label_flatten, w,ignore_index=this.ignore_index,reduction=this.reduction,label_smoothing=this.smooth);
        clsloss_f_=scatter_mean(clsloss_f, instmap);
        clsloss_ = trnf.cross_entropy(outcls, label_flatten, w,ignore_index=this.ignore_index,reduction=this.reduction,label_smoothing=this.smooth);
        clsloss=scatter_mean(clsloss_, instmap);
        # with torch.no_grad():
        #     assert (max(clsloss_-clsloss)==0);
        return clsloss+clsloss_f_;

# since we no more balance unk weights.
class osclsNG_perinstance(abstract_cent):
    DFT_reduction = "none";
    def forward(this, outcls, label_flatten,instmap):
        clsloss_ = trnf.cross_entropy(outcls, label_flatten,ignore_index=this.ignore_index,reduction=this.reduction,label_smoothing=this.smooth);
        clsloss=scatter_mean(clsloss_, instmap);
        # with torch.no_grad():
        #     assert (max(clsloss_-clsloss)==0);
        return clsloss;

# class osclsNG_perinstance_uct(abstract_cent):
#     DFT_reduction = "none";
#     def forward(this, outcls, label_flatten,instmap):
#         lunk=outcls.shape[-1]-1;
#         is_unk=(label_flatten==lunk);
#         not_unk = not is_unk;
#         non_unk=outcls[not_unk, :-1]
#         non_unk_cls=trnf.cross_entropy(,label_flatten[not_unk]);
#         marginal=
#         clsloss_ = trnf.cross_entropy(, label_flatten, w,ignore_index=this.ignore_index,reduction=this.reduction,label_smoothing=this.smooth);
#         clsloss=scatter_mean(clsloss_, instmap);
#         # with torch.no_grad():
#         #     assert (max(clsloss_-clsloss)==0);
#         return clsloss;

class osclsNG_perinstance_soft_softmax(abstract_cent):
    DFT_reduction = "none";
    def forward(this, outcls, label_flatten,instmap):
        with torch.no_grad():
            tard=torch.softmax(outcls,dim=-1)*this.smooth;
            tard+=trnf.one_hot(label_flatten)*(1-this.smooth);
        w = torch.ones(outcls.shape[-1]).to(outcls.device,dtype=outcls.dtype);
        # w[-1] = 0;
        clsloss = trnf.cross_entropy(outcls, tard, w,ignore_index=this.ignore_index,reduction=this.reduction);
...
#         clsloss=scatter_mean(clsloss, instmap);
#         return clsloss;
class osclsNG_perinstance_top20_mk2(abstract_cent):
    DFT_reduction = "none";
    def forward(this, outcls, label_flatten,instmap):
        w = torch.ones(outcls.shape[-1]).to(outcls.device,dtype=outcls.dtype);
        with torch.no_grad():
            outcls__=outcls+0;
            outcls__[torch.nn.functional.one_hot(label_flatten) > 0]=9999;
            topkid=torch.topk(outcls__,20,dim=-1,sorted=True)[1];
            label_flatten_=torch.zeros_like(label_flatten);
            label_flatten_[label_flatten==this.ignore_index]=this.ignore_index;
        outcls_=torch.gather(outcls, dim=-1, index=topkid);
        clsloss_ = trnf.cross_entropy(outcls_, label_flatten_,ignore_index=this.ignore_index,reduction=this.reduction,label_smoothing=this.smooth);
...
class osclsNG_perinstance_top20(abstract_cent):
    ...
    def visualize(this,outcls,scale=4):
        cv2.imwrite("/home/lasercat/writebuffer/tmp/meow.jpg",(outcls*scale).to(torch.uint8).cpu().numpy());
        cv2.waitKey(10);
