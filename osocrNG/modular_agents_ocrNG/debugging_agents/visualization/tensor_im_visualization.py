...
class neko_tensor_im_visualization(neko_module_wrapping_agent):
    ...
    def get_agtcfg(cls,
        ...
        return {"agent": cls,
                "params": {"iocvt_dict": {cls.INPUT_tensor_img: tensor_img, cls.OUTPUT_raw_img_vis: raw_img_vis},
                           cls.PARAM_mean: mean, cls.PARAM_var: var, "modcvt_dict": {}}}

class neko_tensor_im_visualization_nolist(neko_tensor_im_visualization):
    def take_action(this, workspace: neko_workspace, environment: neko_environment):
        tensor_img = workspace.get(this.tensor_img);
        img = [i for i in (tensor_img.detach() * this.var + this.mean).permute(0, 2, 3, 1).cpu().numpy().astype(np.uint8)];
        workspace.add(this.raw_img_vis,img);
