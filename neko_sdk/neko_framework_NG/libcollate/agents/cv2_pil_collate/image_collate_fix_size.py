...
def collate_max_edge_core(cv2_image,target_w,target_h,pad_w,pad_h,pad_val):
...
def collate_max_edge_pil(params):
    ...
    return collate_max_edge_core(cv2_image, target_w, target_h, pad_w, pad_h, pad_val);
def collate_max_edge_cv2(params):
    cv2_image,target_w,target_h,pad_w,pad_h,pad_val=params;
    return collate_max_edge_core(cv2_image, target_w, target_h, pad_w, pad_h, pad_val);

def collate_max_edge_in_parallel_cv2(images,target_w,target_h,pad_w,pad_h,pad_val,pool:Pool):
    ads=[(i,target_w,target_h,pad_w,pad_h,pad_val) for i in images];
    # if (type(images[0]) != type(np.array(0))):
    #     fatal("???");
    return pool.map(collate_max_edge_cv2,ads);
def collate_max_edge_in_parallel_pil(images,target_w,target_h,pad_w,pad_h,pad_val,pool:Pool):
    ads=[(i,target_w,target_h,pad_w,pad_h,pad_val) for i in images];

    return pool.map(collate_max_edge_pil,ads);



class neko_image_collate_fixed_size_max_edge_align_cv2(ama):
    ...
    def take_action(this, workspace: neko_workspace, environment: neko_environment):
        ...
        all_ims=collate_max_edge_in_parallel_cv2(raw_images, this.core_w, this.core_h, this.padding_size_w, this.padding_size_h,
                                         this.padding_val, this.pool);
        tim=torch.tensor(all_ims, dtype=torch.float);
        if(tim.shape[1]==1):
    ...
    def get_agtcfg(cls,
        ...
        return {"agent": cls,
                "params": {"iocvt_dict": {cls.INPUT_raw_images: raw_images, cls.OUTPUT_tensor_images: tensor_images},
                           ...
                           "modcvt_dict": {}}}
class neko_image_collate_fixed_size_max_edge_align_pil( neko_image_collate_fixed_size_max_edge_align_cv2):
    def take_action(this, workspace: neko_workspace, environment: neko_environment):
        raw_images = workspace.get(this.raw_images);
        all_ims=collate_max_edge_in_parallel_pil(raw_images, this.core_w, this.core_h, this.padding_size_w, this.padding_size_h,
                                         this.padding_val, this.pool);
        workspace.add(this.tensor_images,torch.tensor(all_ims, dtype=torch.float));
        return workspace, environment;
