...
class neko_module_opt_setNG:
    ...
    def get_real_mods(this):
        rmd={};
        for k in this.real_modulars:
            rmd[k]=this.real_modulars[k].get_mod(); # if the model does not have a replica, return the main model.
        return rmd;
    ...
    def clear(this):
        ...
        this.real_modulars: Dict[str,neko_modular_NG]= {};
        this.mods={};
        this.replica_groups={};
    ...
    def arm_modules(this, real_modular_cfg:dict[str,Dict], bogo_mod_cfg,decay_override=None):
        this.clear();
    ...
    def tfloat16(this,tag=None):
        for modk in this.real_modulars:
            if ((tag is not None) and
                    (tag not in this.real_modulars[modk].tags)):
                continue;
            this.real_modulars[modk].to(torch.float16);
    ...
    def save(this,nEpoch,batch_idx):
        for modk in this.real_modulars:
            this.real_modulars[modk].force_save_savable(nEpoch, batch_idx);
    def save_as_necessary(this,root,nEpoch, batch_idx):
        for modk in this.real_modulars:
            this.real_modulars[modk].save_as_if_needed(root,nEpoch, batch_idx);
    def save_as(this,root,nEpoch, batch_idx):
        for modk in this.real_modulars:
            this.real_modulars[modk].save_as_if_savable(root,nEpoch, batch_idx);
    def load(this,itrkey):
        for modk in this.real_modulars:
            this.real_modulars[modk].load(modk,itrkey);
    def load_from(this,path,itrkey):
        for modk in this.real_modulars:
            this.real_modulars[modk].load_from(path,modk, itrkey);

    def update_para(this):
        multi_apply(update, this.optimizers);

    def update_d(this, frozen):
        broken_list = [];
        for i in range(0, len(this.optimizers)):
            if i not in frozen:
                tarname=this.optnames[i];
                this.optimizers[i].step();
                if(this.real_modulars[tarname].is_nan()):
                    broken_list.append(tarname)
        if(len(broken_list)==0):
            return True,[];
        else:
            return False,broken_list;
    ...
    def debug_grad(this):
        for modk in this.real_modulars:
            if (this.real_modulars[modk].save_each > 0):
                this.real_modulars[modk].debug_show_grad_norm();
...
def force_arm_bogo_list( bogolist, modcfgs, modular_dict):
    fail_list = [];
    for name in bogolist:
        cfg = modcfgs[name];
        # bogo modules are re-combination of parts of existing real and bogo modules.
...
#
def get_modular_dict(modset):
    modular_dict = {};
    rmd=modset.get_real_mods();
    for k in rmd:
    ...
    if (len(list_bogo_to_arm)):
        ...
        force_arm_bogo_list(list_bogo_to_arm, modset.bogo_config_dict,
                                                                    modular_dict)
    return modular_dict;
