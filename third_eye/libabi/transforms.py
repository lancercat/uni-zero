...
class CVRandomRotation(object):
    ...
    def __call__(this, img):
        ...
        rim=cv2.warpAffine(img, M, (dst_w, dst_h), flags=flags, borderMode=this.border_mode,borderValue=[127,127,127]);
        # cv2.imshow("meow",rim);
        # cv2.imshow("oim",img);
        # cv2.waitKey(0);
...
def get_abi_aug_fixed_extreme(border_mode=cv2.BORDER_REPLICATE):
    return transforms.Compose([
                CVGeometryFixed(degrees=45, translate=(0.0, 0.0), scale=(0.5, 2.), shear=(45, 15), distortion=0.5, p=0.5,border_mode=border_mode),
                CVDeterioration(var=20, degrees=6, factor=4, p=0.25),
                CVColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.1, p=0.25)
