...
def res45_ffn_naive(bochs,fochs,engine=nn.Conv2d):
    ffn_layers={}
    ffn_layers["1"] = {"conv":engine(bochs[1], fochs[1], 1)};
    ffn_layers["2"] = {"conv": engine(bochs[2], fochs[2], 1)};
    ffn_layers["3"] =  {"conv":engine(bochs[3], fochs[3], 1)};
    ffn_layers["4"] =  {"conv":engine(bochs[4],fochs[4],1)};
    ffn_layers["5"] =  {"conv":engine(bochs[5],fochs[5],1)};
    return ffn_layers;
def res45_ffn_normed(bochs,fochs,engine=nn.Conv2d):
    ffn_layers={}
    fatal("notimpl")
    return ffn_layers;
