...
def quasicrystal(height, width):
    ...
    for kw in range(width):
        y = float(kw) / max((width - 1),1) * 4 * math.pi - 2 * math.pi
        for kh in range(height):
            x = float(kh) /  max((height - 1),1)* 4 * math.pi - 2 * math.pi
            z = 0.0
...
def image(height, width, image_dir):
    ...
    if True:
        pic = Image.open(image_dir);

        if pic.size[0] < width:
            pic = pic.resize(
                [width, int(pic.size[1] * (width / pic.size[0]))], Image.LANCZOS
            )
        if pic.size[1] < height:
            pic = pic.resize(
                [int(pic.size[0] * (height / pic.size[1])), height], Image.LANCZOS
            )
...
def image_pic(height, width, pic):
    """
        Create a background with a image
    """
