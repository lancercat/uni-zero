...
class neko_abstract_agent(agent_code_cheat_mixin):
    @classmethod
    def make(cls,cfg) -> Self:
        return cfg[cls.AGT_TYPE_KEY](cfg[cls.AGT_PARA_KEY]);
...
class neko_abstract_async_agent_gen1(neko_abstract_async_agent):
...
# Gen2 does not do remapping by itself.
# the workflow would be more like sync agents, but called without workspace.
# as we want to expose the fact that workspace cannot let gradient go through queues.
#
class neko_abstract_async_agent_gen2(neko_abstract_async_agent):
    EPOCH_END="epoch_end"; # used to trigger some finalization, appears the NEXT iteration after the queue exhausts on epoch.
    RST_NODE="reset_node"; # used to reset a node, appears the NEXT iteration after the queue exhausts on epoch.
    HALT_NODE="halt_node"; # used to a controlled teardown.
    ...
    def setup(this,param):
        super().setup(param);
        this.iqs = neko_get_arg(this.PARAM_INPUT_QUEUES, param, "NEP_skipped_NEP");
        this.oqs = neko_get_arg(this.PARAM_OUTPUT_QUEUES, param, "NEP_skipped_NEP");
        this.inames=neko_get_arg(this.PARAM_INAMES,param,[[] for _  in this.iqs]);
        this.onames=neko_get_arg(this.PARAM_ONAMES,param,[[] for _ in this.oqs]);

    def mount_environment(this,environment:neko_environment):
        this.environment = environment;
    def on_reset(this,workspace,environment):
        pass;
    def on_epoch_end(this,workspace, environment ):
        pass;

    def on_halt(this,workspace,environment):
        this.stop();
    ...
    def action_loop(this):
        this.setup_loopglobal();
        while this.status == this.STATUS_running:
            ws,ctrl = this.iq2ws();
            if ctrl is None:
                # t=time.time();
                this.runstub(ws,this.environment);
                # tt=time.time()-t;
                # print(this.iqs,this.oqs, tt);
                this.ws2oq(ws);
            else:
                if (this.EPOCH_END ==ctrl):
                    this.on_epoch_end(ws,this.environment);
                if (this.RST_NODE==ctrl):
                    this.on_reset(ws,this.environment);
                if(this.HALT_NODE==ctrl):
                    this.on_halt(ws,this.environment);
                this.ctrl2oq(ctrl);
    ...
    def iq2ws(this):
        ...
        ds=[];
        for iq in this.iqs:
            ds.append(this.environment.deque(iq));
        for d, inamelst in zip(ds, this.inames):
            if(this.EPOCH_END in d):
                return None,this.EPOCH_END;
            if(this.RST_NODE in d):
                return None,this.RST_NODE;
            if(this.HALT_NODE in d):
                return None, this.RST_NODE;
            if(inamelst is not None):
                for k in inamelst:
                    ws.add(k, d[k]);
            else:
                for k in d:
                    ws.add(k, d[k]);
        return ws,None
    def ctrl2oq(this,ctrl):
        for oq in this.oqs:
            this.environment.enque(oq, ctrl);
    def ws2oq(this,ws):
        for oq, onamelst in zip(this.oqs,this.onames):
            d = {};
            if(onamelst is None):
                d=ws.inter_dict;
            else:
                for k in onamelst:
                    d[k] = ws.get(k);
            this.environment.enque(oq, d);
    def start(this,environment,mode="forkserver"):
        this.stop();
        this.mount_environment(environment);
        this.launch(mode);
        pass;
    def start_sync(this,environment,mode="NEP"):
        this.stop();
        this.mount_environment( environment);
        this.status = this.STATUS_running;
        this.action_loop();
...
if __name__ == '__main__':
    class neko_gen1dbg(neko_abstract_async_agent_gen1):
        def setup(this, param):
            this.input_dict = {};
            this.output_dict = {};
            this.epoch_cnt = 9;
