...
def render_pred(dict,gt_word,pred_word):
    ...
    if(gt_word is not None):
        if(type(gt_word)==list):
            corids = pylcs.lcs_str2id("".join(gt_word),"".join(pred_word)); # TODO: upgrade this to token pylcs.
    ...
    if(type(gt_word)==list):
        chs=gt_word; # do not tokenize if already tokenized;
    else:
    ...
    for i in range(len(chs)):
        if (i in corids):
            if(flag):
                proto = color(dict, pred_word[i], 0, 255, 0)
            else:
                proto = color(dict, pred_word[i], 255, 255, 255)
        else:
            proto = color(dict, pred_word[i], 0, 0, 255);
