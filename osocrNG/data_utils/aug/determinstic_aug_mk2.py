...
class abiaug_fixed:
    ...
    def process(this,images,thread_pool=None):
        ...
        # return ;
        return l;
class abiaug_fixed_extreme(abiaug_fixed):
    pass;
...
class augment_agent(neko_abstract_async_agent_gen1):
    PARAM_seed="seed";
...
class augment_agent_abinet_fixed(augment_agent):
    ...
    def augagt(cls):
        return abiaug_fixed;
class augment_agent_abinet_fixed_extreme(augment_agent):
    @classmethod
    def augagt(cls):
        return abiaug_fixed_extreme;
...
class neko_basic_beacon_agent(neko_module_wrapping_agent):
    ...
    def take_action(this,workspace:neko_workspace,environment:neko_environment):
        workspace.add(this.o,[cv2.resize(np.array(i),this.size,interpolation=this.mode) for i in workspace.get(this.i)]);
